...
//TODO if(rank==0)  this->JoinPrefetchThread();
    break;  // do nothing
  case DataParameter_DB_LMDB:
//TODO this->JoinPrefetchThread();
if(rank==0){
		...
		case DataParameter_DB_LEVELDB:
			{
				...
				unsigned int slen;
				if(rank==0)slen=iter_->value().ToString().size();
				MPI_Bcast(&slen,1,MPI_INT,0,MPI_COMM_WORLD);
				char* strc=NULL;
				strc = new char[slen];
				if(rank==0)memcpy(strc,iter_->value().data(),slen);
				MPI_Barrier(MPI_COMM_WORLD);
				MPI_Bcast(strc,slen,MPI_CHAR,0,MPI_COMM_WORLD);
				string str(strc,slen);
		...
		case DataParameter_DB_LMDB:
			{
				...
				unsigned int dlen;
				if(rank==0)dlen=mdb_value_.mv_size;
				MPI_Bcast(&dlen,1,MPI_INT,0,MPI_COMM_WORLD);
				char* dbuf=NULL;
				dbuf = new char[dlen];
				if(rank==0)memcpy(dbuf,mdb_value_.mv_data,dlen);
				MPI_Barrier(MPI_COMM_WORLD);
				MPI_Bcast(dbuf,dlen,MPI_CHAR,0,MPI_COMM_WORLD);
				datum.ParseFromArray(dbuf,dlen);
				delete[] dbuf;
			}
			break;
		default:
