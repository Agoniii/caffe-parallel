...
//add by shenbo
  if(solver_param.solver_mode() == caffe::SolverParameter_SolverMode_GPU){
	FLAGS_gpu = rank % Caffe::GetDeviceNum();
  } else {
	FLAGS_gpu = -1;
  }
...
int main(int argc, char** argv) {
  int provided;
 MPI_Init_thread(&argc,&argv,MPI_THREAD_MULTIPLE,&provided);//insert by shenbo
  ...
  if(provided!=MPI_THREAD_MULTIPLE){
	LOG(FATAL)<<"This MPI version is NOT support multi-thread!";
  }
  int ret;
  if (argc == 2) {
