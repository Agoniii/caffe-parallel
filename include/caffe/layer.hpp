...
class Layer {
 public:
  ...
  /**
   * @brief Given the bottom blobs, compute the top blobs and the loss.Like Forward but ONLY for MPI root process to test score.
   *
   * @param bottom
   *     the input blobs, whose data fields store the input data for this layer
   * @param top
  ...
  inline Dtype ForwardTest(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  /**
   * @brief Given the bottom blobs, compute the top blobs and the loss.Like Forward but ONLY for MPI root process.
   *
   * @param bottom
   *     the input blobs, whose data fields store the input data for this layer
   * @param top
...
//      vector<Blob<Dtype>*>* top,const int source);
 ...
 protected:
  ...
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) = 0;
  /** @brief Using the CPU device, compute the layer output. not a pure virtual function because it's only used in data layer*/
...
//      vector<Blob<Dtype>*>* top,int source=0) {;}
  ...
  /**
   * @brief Using the GPU device, compute the layer output.
   *        Fall back to Forward_cpu_test() if unavailable.
   */
...
inline Dtype Layer<Dtype>::ForwardRoot(const vector<Blob<Dtype>*>& bottom,
  ...
  switch (Caffe::mode()) {
  case Caffe::CPU:
    Forward_cpu_root(bottom, top, source);
    break;
  case Caffe::GPU:
    Forward_gpu_root(bottom, top, source);
    break;
  default:
    LOG(FATAL) << "Unknown caffe mode.";
  }
